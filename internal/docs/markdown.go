...
func GenMarkdownTreeCustom(cmd *cobra.Command, dir string, filePrepender, linkHandler func(string, ...string) string) error {
	...
	if !cmd.HasParent() {
		// Root command: place the doc directly in the base dir so it doesn't
		// create a same-named subdirectory (e.g. cli/c8y/c8y/) that would
		// collide with the parent category key in autogenerated sidebars.
		commandParts = []string{}
	...
	categoryDir := filepath.Dir(filename)
	err := os.MkdirAll(categoryDir, 0755)
	if err != nil {
		panic(err)
	}

	// Write _category_.json with a unique key to avoid Docusaurus translation key conflicts
	// when multiple sibling directories share the same name (e.g. assert, applications, versions).
	// Uses cmd.CommandPath() as the key. Because the recursion visits children before the
	// parent, any leaf command that shares a directory with its parent will be overwritten by
	// the parent's write, ensuring the final key always reflects the directory-owning command.
	if categoryDir != dir {
		dirKey := strings.ReplaceAll(cmd.CommandPath(), " ", "-")
		dirLabel := filepath.Base(categoryDir)
		categoryContent := fmt.Sprintf("{\"key\": %q, \"label\": %q}\n", dirKey, dirLabel)
		if err := os.WriteFile(filepath.Join(categoryDir, "_category_.json"), []byte(categoryContent), 0644); err != nil {
			return err
		}
	}
