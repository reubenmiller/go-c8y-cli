...
func WithFirmwareVersion(flagVersion string, flagNameSoftware string, clientFunc func() (*c8y.Client, error)) Option {
				...
				if c8ydata.IsID(softwareName) {
					...
					packages, _, err := client.Inventory.GetManagedObjects(
						...
						&c8y.ManagedObjectOptions{
							Query: fmt.Sprintf("(type eq '%s') and name eq '%s'", "c8y_Firmware", softwareName),
						},
					)
