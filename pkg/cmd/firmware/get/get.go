// v2-based firmware get: the id flag drives iteration (pipe or --id); each
// reference (a firmware package id or name, resolved internally by the SDK) is
// fetched via Repository.Firmware.Get.
package get
...
// NewGetCmd creates a command to Get firmware
func NewGetCmd(f *cmdutil.Factory) *GetCmd {
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get firmware",
		Long:  `Get an existing firmware package (managedObject)`,
		Example: heredoc.Doc(`
...
Get a firmware package (using pipeline)
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-Firmware"),
...
func (n *GetCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return func(ctx context.Context) output.Seq {
			return c8ystream.FromResult(client.Repository.Firmware.Get(ctx, ref, opt))
		}, nil
	})
}
