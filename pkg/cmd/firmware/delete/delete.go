// v2-based firmware delete: the id flag drives iteration (pipe or --id); each
// reference (a firmware package id or name, resolved internally by the SDK) is
// deleted via Repository.Firmware.Delete. --forceCascade (default true) removes
...
Delete a firmware package but keep the binaries
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Remove-Firmware"),
	)
...
func (n *DeleteCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		opt := firmwareitems.DeleteOptions{ForceCascade: in.Bool("forceCascade")}
		return func(ctx context.Context) output.Seq {
			return c8ystream.SubmitStatus(ctx, func(ctx context.Context) op.Result[core.NoContent] {
				return client.Repository.Firmware.Delete(ctx, ref, opt)
			})
		}, nil
	})
}
