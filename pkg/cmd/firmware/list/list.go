// v2-based firmware list: the "query" flag is the iterating input (piped lines
// feed it, or its own value drives a single run); each item builds the typed
// firmwareitems.ListOptions (name/deviceType + an extra raw query) and drives a
// Repository.Firmware.ListAll call whose pages stream through the shared output
// pipeline. The firmware type scoping (type eq c8y_Firmware) is applied by the
// SDK service.
package list
...
Get a list of firmware packages
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-FirmwareCollection"),
...
func (n *ListCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		// Extra filter parts beyond the typed name/deviceType fields: the piped
		// query (with --queryTemplate) and the description filter.
		var parts []string
		...
		return c8ystream.ListCall(rawOutput, opt, client.Repository.Firmware.ListAll), nil
	})
}
