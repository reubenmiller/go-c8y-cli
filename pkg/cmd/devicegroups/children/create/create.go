// v2-based device-group children create: creates a new managed object and links
// it as a child of the selected --childType (addition|asset|device) to a parent
// (resolved scoped to device groups). The id flag drives iteration; the body is
// built from --data/--template (+ --global).
package create
...
Create a child addition and link it to an existing managed object
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("New-DeviceGroupChild"),
