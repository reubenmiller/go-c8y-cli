// v2-based device-group children assign: links an existing managed object as a
// child of the selected --childType (addition|asset|device) to a parent
// (resolved scoped to device groups). The child flag drives iteration; success
// yields no output.
package assign
...
Add a related managed object as a child to an existing managed object
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Add-DeviceGroupChild"),
	)
