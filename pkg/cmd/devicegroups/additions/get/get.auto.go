...
func NewGetCmd(f *cmdutil.Factory) *GetCmd {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
$ c8y devicegroups additions get --group 12345 --child 12345
Get an existing child addition reference
	...
	cmd.Flags().StringSlice("child", []string{""}, "Managed object id (required)")
...
func (n *GetCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	// path parameters
	path := flags.NewStringTemplate("inventory/managedObjects/{group}/childAdditions/{child}")
