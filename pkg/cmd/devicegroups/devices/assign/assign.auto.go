...
// NewAssignCmd creates a command to Assign device to group
func NewAssignCmd(f *cmdutil.Factory) *AssignCmd {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
$ c8y devicegroups devices assign --group 12345 --child 43234
Add a device to a group

$ c8y devicegroups devices assign --group 12345 --child 43234,99292,12222
Add multiple devices to a group

$ c8y devices list | c8y devicegroups devices assign --group 12345
...
func (n *AssignCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	// path parameters
	path := flags.NewStringTemplate("inventory/managedObjects/{id}/childDevices")
