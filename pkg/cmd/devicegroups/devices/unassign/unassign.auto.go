...
// NewUnassignCmd creates a command to Unassign device from group
func NewUnassignCmd(f *cmdutil.Factory) *UnassignCmd {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
$ c8y devicegroups devices unassign --group 12345 --child 22553
...
func (n *UnassignCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	// path parameters
	path := flags.NewStringTemplate("inventory/managedObjects/{group}/childDevices/{child}")
