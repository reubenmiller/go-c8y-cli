// v2-based feature-by-tenant enable: the tenant flag drives iteration (pipe or
// --tenant); the feature (--key) is enabled for each tenant via
// Features.Tenants.EnableForTenant (management tenant). Success yields no output.
package enable
...
// NewEnableCmd creates a command to Enable a feature override for a given tenant
func NewEnableCmd(f *cmdutil.Factory) *EnableCmd {
	...
	cmd := &cobra.Command{
		Use:   "enable",
		Short: "Enable a feature override for a given tenant",
		Long:  `Enable a feature toggle override for a specific tenant (from the management tenant)`,
		Example: heredoc.Doc(`
$ c8y features tenants enable --key example --tenant t12345
Enable a feature for a specific tenant
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Enable-FeatureByTenant"),
	)
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return func(ctx context.Context) output.Seq {
			return c8ystream.SubmitStatus(ctx, func(ctx context.Context) op.Result[core.NoContent] {
				return client.Features.Tenants.EnableForTenant(ctx, key, tenant)
			})
		}, nil
	})
}
