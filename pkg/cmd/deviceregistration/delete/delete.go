// v2-based device registration delete: the id flag drives iteration (pipe or
// --id); each device request id (which is the device's external id, used as-is —
// no name resolution) is removed via Devices.Registration.Delete. Success yields
// no output.
package delete
...
Delete a new device request
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Remove-DeviceRequest"),
	)
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return func(ctx context.Context) output.Seq {
			return c8ystream.SubmitStatus(ctx, func(ctx context.Context) op.Result[core.NoContent] {
				return client.Devices.Registration.Delete(ctx, id)
			})
		}, nil
	})
}
