// v2-based user create: builds the body (userName + profile fields +
// --data/--template) and creates the user via Users.Create. The userName flag
// drives the body (pipe or --userName). The SDK Create has no tenant parameter,
// so --tenant is applied as a per-request override (defaulting to the current
// tenant).
package create
...
// NewCreateCmd creates a command to Create user
...
Create a user using a template
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("New-User"),
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return func(ctx context.Context) output.Seq {
			return c8ystream.Submit(ctx, func(ctx context.Context) op.Result[jsonmodels.User] {
				return client.Users.Create(apiv2.WithTenant(ctx, tenant), body)
			})
		}, nil
	})
}
