// v2-based application delete: the id flag drives iteration (pipe or --id); each
// reference (an application id or name, resolved internally by the SDK) is deleted
// via Applications.Delete. --unsubscribeAll maps to the force query parameter.
package delete
...
func NewDeleteCmd(f *cmdutil.Factory) *DeleteCmd {
	...
	cmd := &cobra.Command{
		...
		Long:  `Delete an existing application`,
		Example: heredoc.Doc(`
...
Delete an application
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Remove-Application"),
	)
...
func (n *DeleteCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		opt := applications.DeleteOptions{Force: in.Bool("unsubscribeAll")}
		return func(ctx context.Context) output.Seq {
			return c8ystream.SubmitStatus(ctx, func(ctx context.Context) op.Result[core.NoContent] {
				return client.Applications.Delete(ctx, ref, opt)
			})
		}, nil
	})
}
