...
// NewUpdateCmd creates a command to Update UI extension details
func NewUpdateCmd(f *cmdutil.Factory) *UpdateCmd {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update UI extension details",
		Long:  `Update details of an existing UI extension`,
		Example: heredoc.Doc(`
$ c8y ui extensions update --id "my-example-app" --availability SHARED
Update an extension's availability to SHARED
        `),
	...
	cmd.Flags().String("id", "", "Extension id (required) (accepts pipeline)")
	cmd.Flags().String("name", "", "Name of the extension")
	cmd.Flags().String("key", "", "Shared secret of the extension")
	cmd.Flags().String("availability", "", "Access level for other tenants. Possible values are : MARKET, PRIVATE (default)")
	cmd.Flags().String("contextPath", "", "contextPath of the extension")
...
func (n *UpdateCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	err = flags.WithPathParameters(
		...
		path,
		inputIterators,
		c8yfetcher.WithUIExtensionByNameFirstMatch(n.factory, args, "id", "id"),
	)
	if err != nil {
		return err
	}
