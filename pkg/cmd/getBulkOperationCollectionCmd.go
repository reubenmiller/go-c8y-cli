...
func (n *getBulkOperationCollectionCmd) getBulkOperationCollection(cmd *cobra.Command, args []string) error {
	...
	return n.doGetBulkOperationCollection(req, outputfile, filters)
}

func (n *getBulkOperationCollectionCmd) doGetBulkOperationCollection(req c8y.RequestOptions, outputfile string, filters *JSONFilters) error {
	...
	if resp != nil && resp.JSONData != nil {
		...
		if isJSONResponse && filters != nil && !globalFlagRaw {
			responseText = filters.Apply(*resp.JSONData, "bulkOperations")
		} else {
