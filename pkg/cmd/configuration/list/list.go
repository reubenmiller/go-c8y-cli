// v2-based configuration list: the "query" flag is the iterating input (piped
// lines feed it, or its own value drives a single run); each item builds a
// Cumulocity inventory query — always scoped to configuration files (type
// c8y_ConfigurationDump) — and drives a typed Configuration.ListAll call whose
...
Get a list of configuration files
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-ConfigurationCollection"),
...
func (n *ListCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		// Always scope to configuration files (type c8y_ConfigurationDump); a plain
		// inventory query would also return unrelated managed objects.
		q.AddFilterEqStr("type", configuration.TypeConfiguration).
			AddFilterEqStr("configurationType", in.String("configurationType")).
			AddFilterEqStr("name", in.String("name")).
			AddFilterEqStr("deviceType", in.String("deviceType")).
			AddFilterEqStr("description", in.String("description"))
		q.AddOrderBy(orderByValue)

		opt := configuration.ListOptions{Query: q.Build()}
		...
		return c8ystream.ListCall(rawOutput, opt, client.Configuration.ListAll), nil
	})
}
