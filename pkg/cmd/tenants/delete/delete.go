// v2-based tenants delete: the id flag drives iteration (pipe or --id); each
// tenant is deleted via Tenants.Delete. When no id is given the current tenant
// is used, matching the spec command's default. Success yields no output.
package delete
...
Delete a tenant by name (from the management tenant)
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Remove-Tenant"),
	)
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return func(ctx context.Context) output.Seq {
			return c8ystream.SubmitStatus(ctx, func(ctx context.Context) op.Result[core.NoContent] {
				return client.Tenants.Delete(ctx, id, tenants.DeleteOptions{})
			})
		}, nil
	})
}
