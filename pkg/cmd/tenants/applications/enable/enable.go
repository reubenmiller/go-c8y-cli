// v2-based tenants applications enable: subscribes an application to a tenant via
// Tenants.SubscribeApplication. The application flag drives iteration (pipe or
// --application); the reference is resolved to an id and written into the body's
// application.id (so piped input populates it via the driver), merged with any
// --data/--template body, then POSTed to the tenant's applications. The tenant
...
// NewEnableCmd creates a command to Subscribe application
func NewEnableCmd(f *cmdutil.Factory) *EnableCmd {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
$ c8y tenants applications enable --tenant "t12345" --application "myMicroservice"
Enable an application of a tenant by name
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Enable-ApplicationByTenant"),
