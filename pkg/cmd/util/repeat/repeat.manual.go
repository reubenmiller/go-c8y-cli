...
func NewCmdRepeat(f *cmdutil.Factory) *CmdRepeat {
	...
	cmd.Flags().Int64Var(&ccmd.randomDelayMin, "randomDelayMin", -1, "random minimum delay in milliseconds")
	cmd.Flags().Int64Var(&ccmd.randomDelayMax, "randomDelayMax", -1, "random maximum delay in milliseconds")
	cmd.Flags().Float32Var(&ccmd.randomSkip, "randomSkip", -1, "randomly skip line based on a percentage, probability as a float: 0 to 1, 1 = always skip, 0 = never skip")
	cmd.Flags().Int64Var(&ccmd.times, "times", 1, "number of times to repeat the input. Defaults to 1")
