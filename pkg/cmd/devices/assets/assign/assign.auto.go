...
func NewAssignCmd(f *cmdutil.Factory) *AssignCmd {
	...
	cmd := &cobra.Command{
		...
		Long:  `Create a child asset reference to an existing device`,
		Example: heredoc.Doc(`
$ c8y devices assets assign --device 12345 --child 44235
Assign a managed object as a child asset to an existing device
        `),
	...
	cmd.Flags().StringSlice("child", []string{""}, "New child asset (required) (accepts pipeline)")
...
func (n *AssignCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	err = flags.WithBody(
		...
		c8yfetcher.WithDeviceByNameFirstMatch(client, args, "child", "managedObject.id"),
	...
	// path parameters
	path := flags.NewStringTemplate("inventory/managedObjects/{device}/childAssets")
