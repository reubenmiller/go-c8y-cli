...
func NewCmdExists(f *cmdutil.Factory) *CmdExists {
	...
	cmd := &cobra.Command{
		Use:   "exists",
		Short: "Assert existance of a device",
		Long: heredoc.Doc(`
			Assert that a device exists or not and pass input untouched
	...
	ccmd.SubCommand = subcommand.NewSubCommand(assertFactory.NewAssertDeviceCmdFactory(cmd, f, assertExists))
