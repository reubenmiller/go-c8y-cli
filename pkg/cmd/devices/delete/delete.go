// v2-based delete command: the id flag drives iteration (pipe or --id slice);
// each id is resolved by go-c8y (name -> id) and deleted via Devices.Delete.
// Success yields no output; errors flow through the shared pipeline.
package delete
...
// NewDeleteCmd creates a command to Delete device
...
$ c8y devices list --type myType | c8y devices delete --workers 5
Delete each piped device concurrently
        `),
