...
func NewAssignCmd(f *cmdutil.Factory) *AssignCmd {
	...
	cmd := &cobra.Command{
		...
		Long:  `Create a child addition reference to an existing device`,
		Example: heredoc.Doc(`
$ c8y devices additions assign --device 12345 --child 44235
Assign a managed object as a child addition to an existing device
	...
	cmd.Flags().StringSlice("child", []string{""}, "New child addition (required) (accepts pipeline)")
