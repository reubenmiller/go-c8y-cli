...
// NewGetCmd creates a command to Get child addition reference
func NewGetCmd(f *cmdutil.Factory) *GetCmd {
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get child addition reference",
		Long:  `Get managed object child device reference`,
		Example: heredoc.Doc(`
$ c8y devices additions get --device 12345 --child 12345
Get an existing child device reference
	...
	cmd.Flags().StringSlice("child", []string{""}, "Device reference id (required)")
