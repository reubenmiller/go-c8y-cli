// v2-based user roles list: lists the available user roles via Userroles.ListAll.
// The endpoint has no filter parameters, so the typed ListOptions carries only
// pagination; the command runs once.
package list
...
// NewListCmd creates a command to Get role collection
...
Get a list of roles
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-RoleCollection"),
		flags.WithOutputType("application/vnd.com.nsn.cumulocity.roleCollection+json", "application/vnd.com.nsn.cumulocity.role+json"),
	)
...
func (n *ListCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return c8ystream.ListCall(rawOutput, opt, client.UserRoles.ListAll), nil
	})
}
