...
// NewCreateCmd creates a command to Submit a SQL query and retrieve the ID of the Dremio job executing this query
...
Create a new datahub job using context
	...
	cmd.Flags().String("sql", "", "The SQL query to execute. The table to query is either referred to with the full path or with the table name if the context defines the path (accepts pipeline)")
...
func (n *CreateCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	// path parameters
	path := flags.NewStringTemplate("service/datahub/dremio/api/v3/sql")
