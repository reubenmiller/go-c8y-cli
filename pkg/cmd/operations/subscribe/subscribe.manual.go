...
Subscribe to operations (in realtime) for all devices, and stop after receiving 10 operations
	...
	cmd.Flags().StringSliceVar(&ccmd.actionTypes, "actionTypes", nil, "Filter by Filter by realtime action types, i.e. CREATE,UPDATE,DELETE")
...
func (n *CmdSubscribe) RunE(cmd *cobra.Command, args []string) error {
	...
	return c8ysubscribe.Subscribe(client, c8y.RealtimeOperations(device), opts)
}
