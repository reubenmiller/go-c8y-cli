// v2-based device profile list: the "name" flag is the iterating input (piped
// lines feed it, or its own value drives a single run); each item builds a
// Cumulocity inventory query — always scoped to device profiles (type
// c8y_Profile) — and drives a typed DeviceProfiles.ListAll call whose pages
...
Get a list of device profiles
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-DeviceProfileCollection"),
...
func (n *ListCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		// Always scope to device profiles (type c8y_Profile); a plain inventory
		// query would also return ordinary devices/managed objects.
		q.AddFilterEqStr("type", deviceprofiles.TypeProfile).
			AddFilterEqStr("name", in.String("name"))
		q.AddOrderBy(orderByValue)

		opt := deviceprofiles.ListOptions{Query: q.Build()}
		...
		return c8ystream.ListCall(rawOutput, opt, client.DeviceProfiles.ListAll), nil
	})
}
