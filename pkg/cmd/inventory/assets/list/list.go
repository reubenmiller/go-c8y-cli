// v2-based inventory assets list (deprecated; use `inventory children list
// --childType asset`): the id flag drives iteration; each parent (resolved as a
// managed object) has its child assets streamed via the ChildAssets service,
// plucking the nested managed objects.
package list
...
// NewListCmd creates a command to Get child asset collection
...
Get a list of the child devices using pipeline
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-ChildAssetCollection"),
...
func (n *ListCmd) RunE(cmd *cobra.Command, args []string) error {
	...
	return r.Run(func(in *c8ystream.Resolver) (c8ystream.Call, error) {
		...
		return c8ystream.ListCall(rawOutput, opt, childref.ListAll(client, childref.TypeAsset, parentID)), nil
	})
}
