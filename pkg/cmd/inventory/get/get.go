// v2-based inventory get: the id flag drives iteration (pipe or --id); each
// managed-object reference (id or name) is fetched via ManagedObjects.Get, which
// resolves the reference internally.
package get
...
// NewGetCmd creates a command to Get managed objects
func NewGetCmd(f *cmdutil.Factory) *GetCmd {
	...
	cmd := &cobra.Command{
		Use:   "get",
		Short: "Get managed objects",
		Long:  `Get an existing managed object`,
		Example: heredoc.Doc(`
...
Get a managed object with parent references
	...
	flags.WithOptions(
		...
		flags.WithPowershellName("Get-ManagedObject"),
