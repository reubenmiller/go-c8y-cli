...
func (r *RequestIterator) GetNext() (*c8y.RequestOptions, interface{}, error) {
	...
	req := &c8y.RequestOptions{
		...
		PrepareRequestOnDryRun: r.Request.PrepareRequestOnDryRun,
	...
	if r.Body != nil && !reflect.ValueOf(r.Body).IsNil() && request.RequestSupportsBody(req.Method) {
		...
		case *mapbuilder.MapBuilder:
			if v.HasFile() {
				...
				r.Logger.Warningf("Evaluating request body")
				// Use a reader for the body which evaluates on read time rather than when the job is queued

				req.Body = v.GetDelayedReader(inputLine)()
