...
Describe -Name "Add-AdditionToDeviceGroup" {
    BeforeEach {

    }

    It -Skip "Add a device to a group" {
        $Response = PSc8y\Add-AdditionToDeviceGroup -Group $Group.id -Child $Device.id
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }

    It -Skip "Add a device to a group by passing device and groups instead of an id or name" {
        $Response = PSc8y\Add-AdditionToDeviceGroup -Group $Group -Child $Device
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }
