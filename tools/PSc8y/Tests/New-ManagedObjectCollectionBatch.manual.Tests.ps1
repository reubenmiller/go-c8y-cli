...
InModuleScope PSc8y {
    Describe -Name "New-ManagedObjectCollectionBatch" {
        ...
        It "Creates a collection of managed objects using fixed data" {
            $rawjson = @"
    {
        "type": "$type",
        "c8y_Kpi": {
...
"@
            # using WhatIf
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -Count 2 -Data $rawjson -WhatIf -InformationVariable request
            ...
            # send request
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -Count 2 -Data $rawjson -InformationVariable request
            $LASTEXITCODE | Should -Be 0
...
"@ | Out-File $jsonfile
            ...
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -Count 2 -Data $jsonfile -WhatIf -InformationVariable request
        ...
        It "Throws an error if the json file contains invalid json" {
            ...
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -AbortOnErrors 1 -Count 2 -Data $jsonfile -WhatIf -InformationVariable request
            Remove-Item $jsonfile -Force
        ...
        It "Creates a collection of managed objects using a jsonnet template" {
...
"@ | Out-File $template
            ...
            # using WhatIf
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -Count 2 -Template $template -WhatIf -InformationVariable request
            $LASTEXITCODE | Should -Be 0
            $Response | Should -BeNullOrEmpty
            $request -match "What If: Sending" | Should -HaveCount 2
            $request -match "Sending \[POST\] request to \[.*/inventory/managedObjects\]" | Should -HaveCount 2
            ...
            # send request
            $Response = New-ManagedObjectCollectionBatch -Workers 5 -Count 2 -Template $template -InformationVariable request
            $LASTEXITCODE | Should -Be 0
            $Response | Should -Not -BeNullOrEmpty
            $Response | Should -HaveCount 2
...
"@ | Out-File $template
            ...
            # using WhatIf
            $Response = New-ManagedObjectCollectionBatch -Workers 1 -AbortOnErrors 1 -Count 2 -Template $template -WhatIf -InformationVariable request
            $LASTEXITCODE | Should -Be 103
            $Response | Should -BeNullOrEmpty

            # send request
            $Response = New-ManagedObjectCollectionBatch -Workers 5 -AbortOnErrors 1 -Count 2 -Template $template -InformationVariable request
            $LASTEXITCODE | Should -Be 103
            $Response | Should -BeNullOrEmpty

            # send request (allow to complete, but with errors)
            $Response = New-ManagedObjectCollectionBatch -Workers 5 -AbortOnErrors 3 -Count 2 -Template $template -InformationVariable request
        ...
        It "Managed object allow setting the processing mode" {
            foreach ($mode in @("PERSISTENT", "QUIESCENT", "TRANSIENT", "CEP")) {
                ...
                $Response = New-ManagedObjectCollectionBatch @options
                $LASTEXITCODE | Should -Be 0
                $Response | Should -BeNullOrEmpty
