...
Describe -Name "Get-TenantStatisticsCollection" {
    ...
    It -Skip "Get tenant statistics collection" {
        $Response = PSc8y\Get-TenantStatisticsCollection
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }

    It -Skip "Get tenant statistics collection for the last 30 days" {
        $Response = PSc8y\Get-TenantStatisticsCollection -DateFrom "-30d" -PageSize 30
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }

    It -Skip "Get tenant statistics collection for the day before yesterday" {
