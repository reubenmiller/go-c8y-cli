...
Describe -Name "Add-ChildAssetToDeviceGroup" {
    BeforeEach {

    }

    It -Skip "Add a device to a group" {
        $Response = PSc8y\Add-ChildAssetToDeviceGroup -Group $Group.id -Child $Device.id
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }

    It -Skip "Add a device to a group by passing device and groups instead of an id or name" {
        $Response = PSc8y\Add-ChildAssetToDeviceGroup -Group $Group -Child $Device
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }
...
" {
        $Response = PSc8y\Get-Device $Device1.name, $Device2.name | Add-ChildAssetToDeviceGroup -Group $Group.id
        $LASTEXITCODE | Should -Be 0
        $Response | Should -Not -BeNullOrEmpty
    }
