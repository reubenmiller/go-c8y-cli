function Add-DeviceCollectionToGroupBatch {
    <#
    .SYNOPSIS
    Add a collection of devices to a device group using client side batching

    .DESCRIPTION
    Add a collection of devices to a device group using client side batching

    .EXAMPLE
    PS> Add-DeviceCollectionToGroupBatch -Group 12345 -InputFile "mylist.csv" -Data @{ custom_data = @{ value = 1 } } -WhatIf
    ...
    #>
        ...
        process {
            foreach ($item in (Expand-DeviceGroup $Group)) {
                if ($item) {
