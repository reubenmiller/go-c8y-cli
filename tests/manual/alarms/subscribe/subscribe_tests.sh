...
test01 () {
    # Watch alarms for a time period
    start_task "seq 1 10 | c8y alarms create --device $mo_id --template ./test.alarm.jsonnet --delay 2s --force"

    values=$( c8y alarms subscribe --device "$mo_id" --duration 10s || true )
...
test02 () {
    # Watch alarms for a number of alarms
    # subscribe to count
    start_task "seq 1 10 | c8y alarms create --device $mo_id --template ./test.alarm.jsonnet --delay 2s --force"

    values=$( c8y alarms subscribe --device "$mo_id" --count 2 || true )
    item_count=$( echo "$values" | c8y util show --select id -o csv | wc -l )
...
test03 () {
    ...
    values=$( c8y alarms subscribe --count 2 || true )
    item_count=$( echo "$values" | c8y util show --select id -o csv | wc -l )
