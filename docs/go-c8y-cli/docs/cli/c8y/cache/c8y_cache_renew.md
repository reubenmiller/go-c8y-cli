---
category: cache
title: c8y cache renew
---
Renew existing cache items

### Synopsis

Renew existing cache items by changing the modified dates of the files to "now" so that the cache has a full time-to-live period

```
c8y cache renew [flags]
```
