---
category: cache
title: c8y cache delete
---
Delete the cache

### Synopsis

Delete existing cache items either partially or cached items older than a time period

```
c8y cache delete [flags]
```
...
```
      --age string   Age duration, i.e. 30s, 2m, 1h, 1d
      --expired      Only removed expired cached items
