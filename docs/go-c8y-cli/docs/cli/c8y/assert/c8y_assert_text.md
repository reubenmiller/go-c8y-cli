...
```
      --exact string    Match exact text (case sensitive)
  -h, --help            help for text
      --input string    input value to be repeated (required) (accepts pipeline)
      --regex string    Match against a regular expression (case sensitive)
      --schema string   Match against a json schema
