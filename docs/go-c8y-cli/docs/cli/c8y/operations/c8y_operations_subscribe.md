---
category: operations
title: c8y operations subscribe
---
Subscribe to realtime operations
...
```
c8y operations subscribe [flags]
```
...
$ c8y operations subscribe --device 12345 --duration 30s
...
```
      --actionTypes strings   Filter by Filter by realtime action types, i.e. CREATE,UPDATE,DELETE
      --count int             Max number of realtime notifications to wait for
      --device string         Device ID
      --duration string       Duration to subscribe for. i.e. 30s, 1m (default "30s")
